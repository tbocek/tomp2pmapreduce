...
/**
 * Convenience method used in the example job to read and split files into certain sizes. Caution: Splitting was abandonned eventually and is not tested currently. The maxFileSize parameter should
 * therefore stay larger than the actual size of the files to simply read it completely and distribute it to the DHT. Probably better to use own implementations instead until this is resolved and
 * tested again.
 * 
...
public class FileSplitter {
	...
	/**
	 * Splits a text file located at keyFilePath into pieces of at max maxFileSize. Words are not split! Meaning, this method is appropriate for word count. Then, file splits are put into the DHT and
	 * a map with all resulting location keys and corresponding {@link FutureMapReduceData} is returned to add listener to them outside.
	 * 
	 * @param keyfilePath
	 * @param dht
